...
import com.pwrteam.logikcore.propositional.builder.FormulaCalculator;
import com.pwrteam.logikcore.propositional.bundle.proof.ProofLine.ProofStep;
import com.pwrteam.logikcore.propositional.bundle.proof.ProofResult.ResultType;
import com.pwrteam.logikcore.propositional.bundle.type.ImplicationRuleType;
import com.pwrteam.logikcore.propositional.bundle.type.QuantifierGeneralizationRuleType;
import com.pwrteam.logikcore.propositional.bundle.type.QuantifierInstantiationRuleType;
import com.pwrteam.logikcore.propositional.bundle.type.ReplacementRuleType;
	...
	public ProofResult build(boolean debug) {
		...
		for(ProofLine proof : proofs) {
			...
			T : switch(rule) {
				...
				case ADDITION: {
					...
					if(proof.getFormula().getPrimary(true).isAction())
						if(proof.getFormula().getPrimary(true).getAction().getType().is(ActionType.OR))
			...
			if(tested.getLast().getFormula().equals(conclusion)) {
				...
				return new ProofResult(ResultType.CORRECT);
		...
		return new ProofResult(ResultType.BAD_RULE);
